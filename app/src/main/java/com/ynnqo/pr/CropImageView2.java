...
public class CropImageView2 extends AppCompatImageView {
    ...
    private static final String TAG = "CropImageView2";
    ...
    private static final int DEFAULT_LINE_COLOR = Color.RED;//0xffff0000;//0xFF00FFFF;
    private static final float DEFAULT_LINE_WIDTH = 2; //dp
    ...
    public void setImageToCrop(Bitmap bmp) {
        ...
        Bitmap bp = Bitmap.createBitmap(bmp.getWidth(),bmp.getHeight(),bmp.getConfig());
        setImageBitmap(bp);
    ...
    public void setImageToCrop2(Bitmap bmp) {

        Bitmap bp = Bitmap.createBitmap(bmp.getWidth(),bmp.getHeight(),bmp.getConfig());
        setImageBitmap(bp);

        //Point[] pts = PlateRecognition.scan2(bmp,handle);
        //setCropPoints(pts);

        result r = PlateRecognition.scan3(bmp,handle);
        setCropPoints(r.getPoints());
        if(!r.getName().equals("")) {
            Toast.makeText(ctx, r.getName(), Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public boolean canRightCrop() {
        ...
        return (pointSideLine(lt, rb, lb) * pointSideLine(lt, rb, rt) < 0)
                && (pointSideLine(lb, rt, lt) * pointSideLine(lb, rt, rb) < 0);
    }
